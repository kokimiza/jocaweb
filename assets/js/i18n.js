...
class I18n {
	...
	#apply(lang) {
		...
		// <title> — body[data-page] でページを特定
		const page = document.body.dataset.page;
		if (page) {
			const pageTitle = this.#resolve(
				this.#catalog,
				`meta.${page}.title.${lang}`,
			);
		...
		document.querySelectorAll("[data-i18n]").forEach((el) => {
			const val = this.#resolve(this.#catalog, `${el.dataset.i18n}.${lang}`);
	...
	#bindEvents() {
		...
		document.addEventListener("click", (e) => {
			...
			// ドロップダウンを閉じる
			document
				.querySelector(".navbar-lang-toggle")
